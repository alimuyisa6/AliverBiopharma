...
const passkeyAdminClient = createClient(
...
export async function handler(req, res, path, ctx) {
  ...
  if (req.method === 'GET' && path === 'passkey_config') return passkeyConfig(req, res);
  if (req.method === 'GET' && path === 'passkey_list') return passkeyList(req, res, ctx);
  if (req.method === 'POST' && path === 'passkey_delete') return passkeyDelete(req, res, ctx);
  if (req.method === 'POST' && path === 'passkey_rename') return passkeyRename(req, res, ctx);
  if (req.method === 'POST' && path === 'passkey_signin') return passkeySignin(req, res);
...
async function passkeyList(req, res, ctx) {
  if (!ctx.authenticated) throw new SecurityError('Authentication required', 401);

  const { data, error } = await passkeyAdminClient.auth.admin.passkey.listPasskeys({
    userId: ctx.userId
  });

  if (error) throw new SecurityError('Unable to load your passkeys', 500);

  return res.status(200).json(
    (data || []).map((passkey) => ({
      id: passkey.id,
      friendly_name: passkey.friendly_name || 'Passkey',
...
async function passkeyDelete(req, res, ctx) {
  if (!ctx.authenticated) throw new SecurityError('Authentication required', 401);

  const body = await parseAndValidateBody(req);
  const passkeyId = typeof body.passkey_id === 'string'
    ? body.passkey_id.trim()
    : '';

  if (!passkeyId || passkeyId.length > 128) {
    throw new SecurityError('Invalid passkey', 400);
  }

  const { error } = await passkeyAdminClient.auth.admin.passkey.deletePasskey({
    userId: ctx.userId,
    passkeyId
  });

  if (error) throw new SecurityError('Unable to remove this passkey', 400);
...
async function passkeyRename(req, res, ctx) {
  if (!ctx.authenticated) throw new SecurityError('Authentication required', 401);
  ...
  if (!friendlyName || friendlyName.length > 120) {
    throw new SecurityError('Passkey name must be between 1 and 120 characters', 400);
  }

  const { data, error } = await passkeyAdminClient.auth.admin.passkey.updatePasskey({
    userId: ctx.userId,
    passkeyId,
    friendlyName
  });

  if (error) throw new SecurityError('Unable to rename this passkey', 400);

  return res.status(200).json({
    id: data?.id || passkeyId,
    friendly_name: data?.friendly_name || friendlyName,
    created_at: data?.created_at || null,
    last_used_at: data?.last_used_at || null
  });
}

async function passkeyConfig(req, res) {
  ...
  return res.status(200).json({
    ...
    publishable_key: process.env.SUPABASE_ANON_KEY,
    origin
  });
}

async function passkeySignin(req, res) {
  const ip = getClientIp(req);
  const body = await parseAndValidateBody(req);
  const { access_token, mfa_code, device } = body;

  if (!access_token || typeof access_token !== 'string' || access_token.length > 4096) {
    throw new SecurityError('Invalid passkey authentication response', 400);
  }

  if (!(await rateLimiter.check(ip, null, 'signin_attempt'))) {
    throw new SecurityError('Too many login attempts. Please try again in 15 minutes.', 429);
  }
  ...
  if (authUserError || !authUserData?.user) {
    await auditLog({ action: 'passkey_signin_failed', ip });
    throw new SecurityError('Passkey authentication failed', 401);
  }
  ...
  if (restriction) {
    ...
    if (restriction.restriction_type === 'suspended') {
      throw new SecurityError(
        restriction.lock_reason || 'Your account has been suspended. Contact support.',
        403
      );
    }
    ...
    if (restriction.restriction_type === 'locked') {
      if (restriction.expires_at && new Date(restriction.expires_at) > new Date()) {
        const hoursLeft = Math.ceil(
          (new Date(restriction.expires_at) - new Date()) / (1000 * 60 * 60)
        );
  ...
  const adminCandidate = await isAdmin(userId, ip);
  ...
  if (adminCandidate?.mfa_enabled) {
    ...
    if (!verifyTotp(adminCandidate.mfa_secret, mfa_code)) {
      await auditLog({ actorId: userId, action: 'mfa_failed', ip });
  ...
  const { data: profile } = await supabase
    .from('user_profiles')
    .select(
      'role, track, class_name, onboarding_completed, is_approved_teacher, approved_track, active_level_id, active_group_id, profile_picture_url, theme_color, accessibility, language, timezone, preferences'
    )
    .eq('user_id', userId)
    .maybeSingle();
  ...
  await createNotification(userId, 'new_login', {
    location: 'Unknown',
...
async function signin(req, res) {
  const ip = getClientIp(req);
  const body = await parseAndValidateBody(req);
  const { email, password, turnstile_token, mfa_code, passkey_token, device } = body;
  ...
  const { data: profile } = await supabase
    .from('user_profiles')
    .select('role, track, class_name, onboarding_completed, is_approved_teacher, approved_track, active_level_id, active_group_id, profile_picture_url, theme_color, accessibility, language, timezone, preferences')
    .eq('user_id', data.user.id)
    .maybeSingle();
  ...
  const session = await createUserSession(data.user.id, ip, req.headers['user-agent'], req, mfaWasVerified, isAdminSession, passkeyWasVerified, device);
...
async function changePassword(req, res, ctx) {
  ...
  const body = await parseAndValidateBody(req);
  const { current_password, new_password, captcha_token } = body;

  if (!current_password || !new_password) throw new SecurityError('Current and new password are required', 400);
  if (!captcha_token || typeof captcha_token !== 'string' || captcha_token.length > 4096) {
...
async function createUserSession(userId, ip, userAgent, req, mfaVerified = false, isAdminSession = false, passkeyVerified = false, clientDevice = null) {
  ...
  const deviceMetadata = getDeviceMetadata(req, clientDevice);
  ...
  const { error } = await supabase.from('user_sessions').insert({
    ...
    device_platform: deviceMetadata.device_platform,
    device_os_version: deviceMetadata.device_os_version,
    device_browser: deviceMetadata.device_browser,
    device_browser_version: deviceMetadata.device_browser_version,
