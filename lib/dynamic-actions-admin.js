...
function validateAction(body, partial = false) {
  if (!partial || body.action_key !== undefined) {
    if (typeof body.action_key !== 'string' || !/^[a-z0-9][a-z0-9_.-]{1,99}$/.test(body.action_key)) {
...
export async function handler(req, res, path, ctx) {
  ...
  if (path === 'create') {
    ...
    const { data, error } = await supabase.from('dynamic_actions').insert({ action_key, label: label.trim(), ...rest }).select('*').single();
    if (error) throw new SecurityError(error.code === '23505' ? 'action_key already exists' : 'Failed to create action', error.code === '23505' ? 409 : 500);
    return res.status(201).json(data);
  }

  if (path === 'update') {
    if (typeof body.id !== 'string') throw new SecurityError('id required', 400);
    validateAction(body, true);
    const allowed = ['action_key','label','description','action_type','destination','icon','variant','page_id','is_enabled','visibility_mode','auth_required','requires_premium','required_role','level_ids','group_ids','content_type','content_id','starts_at','ends_at','display_order','config'];
    const updates = Object.fromEntries(allowed.filter(k => body[k] !== undefined).map(k => [k, k === 'label' ? body[k].trim() : body[k]]));
    if (!Object.keys(updates).length) throw new SecurityError('No changes supplied', 400);
    updates.updated_at = new Date().toISOString();
    const { data, error } = await supabase.from('dynamic_actions').update(updates).eq('id', body.id).select('*').single();
    if (error) throw new SecurityError(error.code === '23505' ? 'action_key already exists' : 'Failed to update action', error.code === '23505' ? 409 : 500);
    return res.status(200).json(data);
  }
