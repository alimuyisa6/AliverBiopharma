...
async function listTutors(req, res, ctx) {
  ...
  let query = supabase
    .from('tutor_profiles')
    .select(`
      id, user_id, display_name, headline, bio, years_experience, country, district, teaching_mode, languages, hourly_rate, profile_views, search_vector,
      avatar_url,
      user_profiles!inner (
        profile_picture_url
      )
  ...
  const profiles = data || [];
  const profileIdsForUnits = profiles.map((profile) => profile.id);

  let curriculumByProfile = {};
  if (profileIdsForUnits.length) {
    const { data: profileUnits } = await supabase
      .from('tutor_profile_units')
      .select(`
        profile_id,
        unit_id,
      ...
      `)
      .in('profile_id', profileIdsForUnits)
      .in('unit_id', allowedUnitIds);

    for (const row of profileUnits || []) {
      const unit = row.curriculum_units;
      const group = unit?.curriculum_groups;
      const level = group?.curriculum_levels;
      if (!unit || !group || !level) continue;

      if (!curriculumByProfile[row.profile_id]) {
        curriculumByProfile[row.profile_id] = {
          units: [],
          groups: [],
          levels: [],
        };
      ...
      const bucket = curriculumByProfile[row.profile_id];

      if (!bucket.units.some((item) => item.id === unit.id)) {
        bucket.units.push({
          id: unit.id,
          name: unit.name,
          group_id: group.id,
      ...
      if (!bucket.groups.some((item) => item.id === group.id)) {
        bucket.groups.push({
          id: group.id,
          name: group.name,
          level_id: level.id,
          level_name: level.display_name,
          label: level.group_label,
        });
      }

      if (!bucket.levels.some((item) => item.id === level.id)) {
        bucket.levels.push({
          id: level.id,
  ...
  const tutors = profiles.map((tutor) => ({
    ...tutor,
    avatar_url: tutor.avatar_url || tutor.user_profiles?.profile_picture_url || null,
    curriculum: curriculumByProfile[tutor.id] || { units: [], groups: [], levels: [] },
  }));
...
async function getTutorDetail(req, res, ctx) {
  ...
  let profileQuery = supabase
    .from('tutor_profiles')
    .select(`
      id, user_id, display_name, headline, bio, years_experience, country, district, teaching_mode, languages, hourly_rate, verification_status, profile_views,
      avatar_url,
      user_profiles!inner (
        profile_picture_url
      )
...
async function sendContactRequest(body, res, ctx) {
  ...
  const { error: insertError } = await supabase.from('tutor_contact_requests').insert({
    tutor_id,
  ...
  if (insertError) {
    console.error('[Tutors] contact request insert failed:', insertError.message);
    throw new SecurityError('Could not create the tutor contact request', 500);
  }
