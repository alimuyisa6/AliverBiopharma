...
async function getActiveUnitIds(ctx) {
  ...
  // A protected PDF request must have a valid, server-resolved
  // curriculum scope. Never fall back to the client-supplied track/class.
  if (!scope || scope.pending || !scope.active_group_id) {
    return [];
  }
  ...
  if (error) {
    throw new SecurityError('Failed to resolve PDF curriculum scope', 500);
  }
