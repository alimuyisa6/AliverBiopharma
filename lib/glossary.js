  ...
  if (mappings && mappings.length > 0) {
    ...
    for (const [table, ids] of Object.entries(tableIds)) {
      ...
      if (table === 'notes') {
        ...
        query = supabase
          .from('quizzes')
          .select('id, title, description, category, difficulty, subject, unit_id')
    ...
    if (allowedUnitIds !== null) {
      ...
      content.quizzes = scopeFilter(content.quizzes);
      content.flashcards = scopeFilter(content.flashcards);
    }
