...
async function getRecentActivity(userId, unitIds = []) {
  ...
  const activityUnitIds = [...new Set([
    ...unitIds,
    ...(quizActs.data || []).map((row) => row.unit_id).filter(Boolean)
  ])];

  let quizUnitMap = new Map();

  if (activityUnitIds.length) {
    const { data: units } = await supabase
      .from('curriculum_units')
      .select('id, name, slug, group_id')
      .in('id', activityUnitIds);

    quizUnitMap = new Map((units || []).map((unit) => [unit.id, unit]));
  }
  ...
  for (const row of quizActs.data || []) {
    feed.push({
      ...
      details: `${quizUnitMap.get(row.unit_id)?.name || 'Quiz'} block ${Number(row.block_number) + 1}`,
      unit_id: row.unit_id || null,
      group_id: quizUnitMap.get(row.unit_id)?.group_id || null,
      slug: quizUnitMap.get(row.unit_id)?.slug || null,
...
async function getPerUnitXp(userId, unitIds) {
  ...
  const { data: units } = await supabase
    .from('curriculum_units')
    .select('id, name, slug, group_id')
    .in('id', Object.keys(unitXp));

  const unitMap = new Map((units || []).map((unit) => [unit.id, unit]));

  return Object.entries(unitXp).map(([unit_id, xp]) => ({
    unit_id,
    unit_name: unitMap.get(unit_id)?.name || 'Unit',
    slug: unitMap.get(unit_id)?.slug || null,
    group_id: unitMap.get(unit_id)?.group_id || null,
    xp
  }));
}
...
async function getRecommendations(userId, unitIds) {
  ...
  const dueQuestionIds = [...new Set((dueReviews.data || []).map((review) => review.question_id).filter(Boolean))];
  let dueQuestionUnitMap = new Map();

  if (dueQuestionIds.length) {
    const { data: questions } = await supabase
      .from('quiz_questions')
      .select('id, unit_id')
      .in('id', dueQuestionIds);

    dueQuestionUnitMap = new Map((questions || []).map((question) => [question.id, question.unit_id]));
  }
  ...
  for (const review of dueReviews.data || []) {
    recommendations.push({
      ...
      unit_id: dueQuestionUnitMap.get(review.question_id) || null,
      priority: 'medium'
    });
  }
...
async function getMasteryMap(userId, unitIds) {
  if (!unitIds.length) return [];

  const [{ data: units }, { data: attempts }, { data: weakAreas }] = await Promise.all([
    supabase
      .from('curriculum_units')
      .select('id, name, slug, group_id')
      .in('id', unitIds)
    ...
    supabase
      .from('quiz_attempts')
      .select('unit_id, percentage, submitted_at')
      .eq('user_id', userId)
      .in('unit_id', unitIds)
      .not('submitted_at', 'is', null)
      .order('submitted_at', { ascending: false })
    ...
    supabase
      .from('user_weak_concepts_v2')
      .select('unit_id, incorrect_attempts')
      .eq('user_id', userId)
      .in('unit_id', unitIds)
      .eq('resolved', false)
      .limit(500)
  ...
  for (const row of attempts || []) {
    if (!attemptMap.has(row.unit_id)) attemptMap.set(row.unit_id, []);
    attemptMap.get(row.unit_id).push(Number(row.percentage) || 0);
  }

  const weakMap = new Map();
  for (const row of weakAreas || []) {
    weakMap.set(row.unit_id, (weakMap.get(row.unit_id) || 0) + (Number(row.incorrect_attempts) || 0));
  }

  return (units || []).map((unit) => {
    const scores = attemptMap.get(unit.id) || [];
    const assessmentCount = scores.length;
    ...
    const weaknessPenalty = Math.min(25, (weakMap.get(unit.id) || 0) * 2);
    const mastery = Math.max(0, Math.min(100, average - weaknessPenalty));
    ...
    return {
      ...
      status: mastery >= 80 ? 'Strong' : mastery >= 60 ? 'Developing' : 'Needs attention'
    };
  }).sort((a, b) => {
