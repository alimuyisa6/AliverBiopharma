...
async function handleGet(path, req, res, ctx) {
  switch (path) {
    ...
    case 'get_resource_ratings':
      requireSuperAdmin(ctx);
      return getResourceRatings(req, res);
    default:
...
async function handlePost(path, body, req, res, ctx) {
  switch (path) {
    ...
    case 'approve_resource':
      requireSuperAdmin(ctx);
      return reviewResourceSubmission(body, res, ctx);
    case 'reject_teacher':
      requireSuperAdmin(ctx);
...
async function getStats(req, res) {
  const [
    ...
    supabase.from('pdf_resources').select('id', { count: 'exact', head: true }).eq('is_active', true),
...
async function getResources(req, res) {
  const [{ data: notes }, { data: pdfs }] = await Promise.all([
    supabase.from('notes')
      .select('id, title, author, category, tag, section_type, unit_id, is_premium, is_active, download_count, created_at, updated_at')
      .eq('is_active', true).order('created_at', { ascending: false }).limit(500),
    supabase.from('pdf_resources')
      .select('id, title, author, unit_id, is_premium, is_active, download_count, created_at, updated_at')
      .eq('is_active', true).order('created_at', { ascending: false }).limit(500)
  ]);
  return res.status(200).json([
    ...(notes || []).map((note) => ({ ...note, resource_type: 'note' })),
    ...(pdfs || []).map((pdf) => ({ ...pdf, resource_type: 'pdf' }))
  ]);
}

async function getResourceRatings(req, res) {
  const { data, error } = await supabase.from('content_ratings').select('content_type, content_id, rating');
  if (error) throw new SecurityError('Failed to fetch resource ratings', 500);
  const result = {};
  for (const row of data || []) {
    const key = String(row.content_id);
    if (!result[key]) result[key] = { total: 0, sum: 0, count: 0, avg: 0 };
    result[key].total += 1;
    result[key].sum += Number(row.rating) || 0;
    result[key].count += 1;
  }
  for (const key of Object.keys(result)) result[key].avg = result[key].count ? result[key].sum / result[key].count : 0;
...
async function getNewsletterSubscribers(req, res) {
  const { data, error } = await supabase
    ...
    .select('email, status, subscribed_at, unsubscribed_at, user_id')
...
async function reviewResourceSubmission(body, res, ctx) {
  const { submissionId, action = 'approve', notes } = body;
  if (!submissionId) throw new SecurityError('submissionId required', 400);
  if (!['approve', 'reject', 'delete'].includes(action)) throw new SecurityError('Invalid resource action', 400);
  ...
  if (action === 'delete') {
    ...
    if (submission.submitted_by) {
      await createNotification(submission.submitted_by, action === 'approve' ? 'resource_approved' : 'resource_rejected', { submissionId, notes: notes || null });
    }
  }
  ...
  await auditLog({
    actorId: ctx.userId,
    actorRole: ctx.adminData?.admin_role,
    action: action === 'approve' ? 'approve_resource' : action === 'reject' ? 'reject_resource' : 'delete_resource_submission',
