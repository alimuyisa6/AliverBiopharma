...
async function requestDataExport(res, ctx) {
  ...
  try {
    ...
    if (uploadError) {
      throwSupabaseError('requestDataExport:upload', 'storage.uploads', uploadError, userId);
    }
...
async function requestAccountDeletion(res, ctx) {
  ...
  if (filePaths.length) {
    ...
    if (storageError) console.error('[PROFILE] account deletion storage cleanup failed', storageError);
  }
