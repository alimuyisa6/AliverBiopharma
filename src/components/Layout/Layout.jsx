...
<span style={{ ...barBase, top: 0, width: fullWidth + 'px', transform: isOpen ? `translateY(${centerY}px) rotate(45deg)` : 'translateY(0) rotate(0deg)' }} />
<span style={{ ...barBase, top: centerY + 'px', width: halfWidth + 'px', opacity: isOpen ? 0 : 1 }} />
<span style={{ ...barBase, bottom: 0, width: fullWidth + 'px', transform: isOpen ? `translateY(-${centerY}px) rotate(-45deg)` : 'translateY(0) rotate(0deg)' }} />
</span>
);
}
...
return (
<span style={{ position: 'relative', display: 'inline-block', width: size + 'px', height: size + 'px' }}>
<span style={{ position: 'absolute', top: 0, left: 0, width: ringSize + 'px', height: ringSize + 'px', border: `${ringThickness}px solid ${GREY}`, borderRadius: '50%', boxSizing: 'border-box' }} />
<span style={{ position: 'absolute', top: handleOffset + 'px', left: handleOffset + 'px', width: handleThickness + 'px', height: handleLength + 'px', background: GREY, borderRadius: handleThickness + 'px', transform: 'rotate(45deg)', transformOrigin: 'top left' }} />
</span>
);
}
...
<span key={rotation} style={{ position: 'absolute', top: rayOffset + 'px', left: 0, width: rayLength + 'px', height: rayThickness + 'px', background: GREY, borderRadius: rayThickness + 'px', transform: `rotate(${rotation}deg)` }} />
))}
<span style={{ position: 'absolute', top: offset + 'px', left: offset + 'px', width: coreSize + 'px', height: coreSize + 'px', borderRadius: '50%', background: GREY }} />
</span>
);
}
...
const isNoFooterPage = NO_FOOTER_PATHS.some((path) => location.pathname === path);
const hideHeader = isAuthPage || isNoteDetailPage || isNoChromePage;
const hideFooter = isAuthPage || isRoomPage || isNoteDetailPage || isNoChromePage || isNoFooterPage;
...
return () => { window.removeEventListener('scroll', handler); clearTimeout(persistTimeout.current); };
}, [routeKey]);

useLayoutEffect(() => {
const restore = () => {
if (navigationType === 'POP' && scrollPositions.current.has(routeKey)) window.scrollTo(0, scrollPositions.current.get(routeKey));
...
setSigningOut(true);
try { await signout(); await refreshUser(); navigate('/'); }
catch { navigate('/'); }
...
if (blockedHeaderPaths.includes(link.href)) return false;
...
{filteredNavigation.map((link) => <Link key={link.href} to={link.href} className="mobile-nav-link" onClick={() => setMobileOpen(false)}>{link.icon && <Icon name={link.icon} />}{link.label}</Link>)}
...
{!hideFooter && showFooter && (<footer className="footer">
<div className="footer-wave" aria-hidden="true"><svg viewBox="0 0 1440 120" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg"><path d="M0,64 C240,120 480,0 720,32 C960,64 1200,112 1440,48 L1440,120 L0,120 Z" className="footer-wave-path" /></svg></div>
<div className="footer-inner">
<div className="footer-brand"><Link to="/" className="header-logo">{logo ? <img src={logo} alt={siteName} className="footer-logo" /> : siteName}</Link><p className="footer-tagline">Advancing biology and pharmacy education for every learner.</p>
{footer.social_links && Object.keys(footer.social_links).length > 0 && (<div className="footer-social">{Object.entries(footer.social_links).map(([platform, url]) => <a key={platform} href={url} target="_blank" rel="noopener noreferrer" className="footer-social-link" data-platform={platform} aria-label={platform}><Icon name={platform} /></a>)}</div>)}
</div>
...
<div className="footer-bottom"><p className="footer-copyright">&copy; {new Date().getFullYear()} AliverBiopharm. All rights reserved.</p><nav className="footer-bottom-nav"><Link to="/privacy" className="footer-bottom-link">Privacy</Link><Link to="/terms" className="footer-bottom-link">Terms</Link><Link to="/about" className="footer-bottom-link">About</Link></nav></div>
</footer>)}
</div>
);
}
